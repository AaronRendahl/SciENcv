read_projects <- function(file, sheet="projects") {
  ...
  d2b <- readxl::read_excel(file, "projects", skip=kb[1], col_names=names(d2a), n_max = diff(range(kb))+1)
  kc <- which(str_detect(d2a$var, "^personmonths_[0-9]+$"))
  d2c <- readxl::read_excel(file, "projects", skip=kc[1], col_names=names(d2a), n_max = diff(range(kc))+1)
...
projects_to_xml <- function(dat) {
  rowtolist <- function(drow) {
    ...
    com <- drow |> select(starts_with("personmonths_")) |>
      pivot_longer(everything(), names_to=c("X", "year"), names_sep="_") |>
      filter(!is.na(value))
    foo <- lapply(seq_len(nrow(com)), \(idx) {
      list(personmonth = structure(list(com$value[idx]), year = com$year[idx]))
    })
    out <- drow |> select(!starts_with("personmonths_")) |> lapply(as.list1)
    out$commitment <- foo
    out
  }
